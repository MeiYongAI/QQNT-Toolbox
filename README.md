...
- 校验完成的更新只会进入 `data\qqnt_toolbox\updater` 暂存目录，不会覆盖正在运行的插件。
- 完全退出 QQ 后，临时 PowerShell 安装器才会替换插件目录；替换失败会恢复原目录，成功后仍在 `updater\backups` 中保留上一版备份。
...
- 删除消息气泡装扮。
- 隐藏天气、经典模式、帮助、锁定、退出账号、检查更新及更新通知等入口。
- 隐藏 VIP 彩色昵称。
