...
- 发现更高版本后，点击“下载更新”会下载 Release ZIP，并校验资产大小、SHA256、ZIP 路径、插件标识、版本号和关键运行文件。
...
该功能会在离屏区域复制、清理并重新排版 QQ 已渲染的消息 DOM，而不是截取当前窗口。它可覆盖文本、图片、视频缩略图、文件、语音、回复、卡片及混合消息，并会排除 QQ 多选灰层、`+1` 和 Toolbox 控件；是否保留表情回应可单独设置。尚未加载完成的媒体会按当时界面中的加载状态输出。

生成结果会直接保存到指定目录，默认位置为 `data\qqnt_toolbox\message-images`，同名文件会自动追加序号。设置中可以修改保存目录和命名格式，并使用 `{yyyy}`、`{MM}`、`{dd}`、`{HH}`、`{mm}`、`{ss}`、`{count}` 作为时间及消息数量占位符；还可选择生成后自动复制到剪贴板。保存完成后会以轻量提示显示结果，不再弹出另存为窗口。

点击“管理图片”可打开独立管理页面，对已保存图片进行预览、复制、重命名、删除、分类和排序。分类信息只记录在插件数据中，不会改变图片在保存目录中的实际层级。
...
开启“消息相关 > 伪造合并转发”后，聊天输入功能栏会显示对应入口。可为每条消息填写发送者 QQ、显示昵称、时间、文本和图片，支持纯图片及文字与多图组合，并可调整消息顺序后发送到当前好友或群聊。
...
- “Telegram”负责下载贴纸包，并直接显示转换工具与网络配置。
...
- TG 样式媒体预览：以单个独立全屏窗口接管 QQ 原本的图片和视频打开请求；未下载媒体会继续调用 QQ 原生下载，并只预加载当前项前后各一项。支持按钮、方向键或滚轮切换媒体，使用 `Ctrl + 滚轮` 缩放，放大后可拖动，右下角可旋转媒体。单击空白或按 `Esc` 关闭，单击视频切换播放；可在缩进子项中选择透明、白色、半透明或黑色背景。
...
├─ message-image-library.json  # 消息图片分类与排序信息
├─ debug                       # 启用诊断记录后按需创建
