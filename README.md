...
开启“消息相关 > 移除表情回应限制”后，会在 QQ 原生回应面板中补回默认被隐藏的 Unicode emoji，不再限制面板内可选的回应表情。表情 ID、名称与图片直接读取 QQ 数据目录中的 `face_config.json` 和 `emoji_res`，不会下载或内置另一套表情包；最近表情及其排序仍完全由 QQ 管理。

### 回应后不关闭回应窗口

开启“消息相关 > 回应后不关闭回应窗口”后，点击任意原生或补全表情都不会关闭回应面板，便于连续添加或取消回应。该开关独立于“移除表情回应限制”；关闭时，补全表情也会与 QQ 原生表情一样在回应后收起面板。
