...
拍一拍使用 Windows x64 原生桥接，并在当前 `wrapper.node` 的函数入口中自动匹配唯一签名。QQ `9.9.25-42905` 已通过实机验证，官方安装包中的 `9.9.21-39038` 和 `9.9.31-49599` 已通过静态签名验证；采用相同内部实现的中间版本也可以自动适配。签名不存在或不唯一时桥接会拒绝安装，不会尝试未知地址。其他操作系统上的拍一拍功能不会生效，其余 Toolbox 功能不受此限制。
