...
“群文件防撤回下载”使用独立群白名单。只有功能已开启、消息来自白名单群、文件大小落在配置闭区间内时才会自动下载；默认范围为 `0.5 KB ～ 600 MB`，按 1024 进制换算，边界包含在内。文件名、扩展名、MIME 和内容类型不参与判定，ZIP、JS、TXT 和无扩展名文件按同一规则处理。未撤回文件只保留在暂存区，发生撤回后才进入持久归档，并一直保留到手动清理。归档后的文件气泡继续使用 QQ 原生“已下载”、左键打开和“在 Finder 中显示”交互，只额外保留撤回红边。
