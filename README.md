...
- 窗口内查看媒体：图片和视频直接覆盖显示在当前 QQ 窗口中，不再打开独立预览窗口；未下载媒体在打开或切换到该项时会调用 QQ 原生下载，并只预加载当前图片前后各一张。支持按钮、方向键或滚轮切换前后媒体，使用 `Ctrl + 滚轮` 缩放，图片放大后可拖动查看。图片点击任意非切换控件区域关闭，视频点击遮罩空白处或按 `Esc` 关闭；可在缩进子项中选择透明、白色、半透明或黑色背景。
- 以图片方式打开表情：开启后，点击消息中的商城表情图像会调用 QQ 原生图片查看流程；若同时开启窗口内查看媒体，则由窗口内媒体查看器承接。普通小黄脸、头像、回复缩略图和消息空白区域不受影响。
- 单击查看媒体：只把消息中图片和视频（包括 Markdown 消息内的图片）的打开手势改为单击，不指定查看器；使用 QQ 原查看器或“窗口内查看媒体”时均独立生效。
